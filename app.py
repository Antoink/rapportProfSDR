...
report_mode = st.radio("Type de rapport", ["Préparation Physique", "Kiné / Prévention"], horizontal=True)
...
GROUPES = GROUPES_PREPA if report_mode == "Préparation Physique" else GROUPES_KINE
...
def get_norm_info(label, value, use_rel=False):
    # La fonction puise maintenant dans les bons dictionnaires
...
def get_kine_radar_pct(label, val, df_ref, col, use_rel, row_data):
    ...
    # Sélection du dictionnaire selon le mode
    norm_dict = NORMES_RELATIVES if use_rel else NORMES_ABSOLUES
    target = norm_dict.get(label_clean)
    
    # Si pas de norme définie, on prend la médiane du groupe (repli)
    if target is None:
    ...
    # Calcul dynamique :
    # Si on est en relatif, la cible brute = target (N/kg) * poids du joueur (kg) = N (Newton)
    # On compare toujours des Newtons (valeur réelle du test) avec la cible calculée
    cible_brute = target * row_data.get("Poids (kg)", 1) if use_rel else target
    
    if is_inverted(label):
        # Pour les temps/chronos (plus bas = mieux)
        pct = (cible_brute / val) * 100
    else:
        # Pour la force/puissance (plus haut = mieux)
        pct = (val / cible_brute) * 100
        
    return max(50, min(pct, 150)) # Plafonnage 50-150%
...
def build_prepa_report(player_name, row, df_ref, df_full, poste, latéralité, anthro, selected_metrics, use_relative, radar_labels, radar_values, themes, dominant, weak, strat_salle, strat_terrain, photo_b64, logo_b64, logo_ext, staff_evals, current_session, df_prev_session, rdv_date, entretien_date, context_test, ref_group_label, comp_zscores):
    ...
    radar_html = f'<div class="no-break" style="text-align:center; margin: 20px 0;"><img src="data:image/png;base64,{radar_b64}" style="width:100%; max-width:450px;"></div>' if radar_b64 else "<p style='font-size:8pt; color:#999; text-align:center;'>Aucune variable sélectionnée pour le radar.</p>"
...
players = sorted(df_session["Joueur"].dropna().unique().tolist())
if not players:
    st.warning("Aucun joueur trouvé pour cette sélection.")
    st.stop()
p_sel = st.sidebar.selectbox("Joueur à profiler", players)
row = df_session[df_session["Joueur"] == p_sel].iloc[0]
...
if report_mode == "Kiné / Prévention":
    st.sidebar.markdown("---")
...
choix_niveau = st.sidebar.selectbox("Niveau hiérarchique", ["Club entier", "Équipe", "Poste large", "Position précise"])
...
if player_metrics_key not in st.session_state:
    ...
    for grp, lbls in GROUPES.items():
        # On empêche l'auto-sélection du Biodex en mode Kiné
    ...
    with st.sidebar.expander(group, expanded=False):
        for label in labels:
            ...
            if checked:
                ...
                statut_def, obj_def = auto_eval_metric(label, v_metric, v_pct)
                    
                statut_opts = ["Acquis", "Proche", "Non Acquis"]
                idx_statut = statut_opts.index(statut_def) if statut_def in statut_opts else 0
                ...
                if statut != "Acquis":
                    if report_mode == "Préparation Physique":
                        obj_val = st.text_input("Objectif", value=obj_def, key=f"obj_{label}_{key_suffix}")
                    else:
                        ...
                        with c_delai:
                            ...
                            delai_val = st.selectbox("Délai", delai_opts, index=0, key=f"delai_{label}_{key_suffix}")
...
comp_zscores = {}
for group, labels in GROUPES.items():
    if group == "Ratio Mixte" and report_mode != "Kiné / Prévention": continue
    z_list = []
    for label in labels:
...
with c1:
    ...
    if report_mode == "Kiné / Prévention" and antecedents_kine:
        prioritaires_ui = []
    ...
    for group, labels in sorted_groups_ui:
        ...
        for label in labels:
            ...
            eval_data = staff_evals.get(label, None)
            group_cards_html += get_metric_card_html(label, col, value, unit, pct, z_score, eval_data, prev_val, use_rel, is_report=False, is_kine=(report_mode == "Kiné / Prévention"))
...
with c2:
    st.markdown("**Variables pour le radar (6 à 8 conseillées)**")
    if report_mode == "Préparation Physique":
        radar_options = sorted([m for m in selected_metrics if m in GROUPES_PREPA.get("Force", []) or m in GROUPES_PREPA.get("Puissance", []) or m in GROUPES_PREPA.get("Saut", [])])
        default_radar = radar_options[:8]
        radar_selection = st.multiselect("Choix radar", radar_options, default=default_radar, label_visibility="collapsed", key=f"radar_sel_{key_suffix}")

        radar_labels, radar_values, radar_vals_d = [], [], None
        for label in radar_selection:
            col = find_column(df_session, label)
            use_rel = st.session_state.use_relative.get(label, False)
            value = get_value_for_metric(row, df_session, col, use_rel) if col else None
        ...
        if radar_labels:
            radar_b64_preview = create_radar_chart(radar_labels, radar_values)
            ...
            st.caption("Sélectionne au moins une variable numérique pour afficher le radar.")
    else:
        ...
        radar_options = sorted(list(base_kine_metrics))
        default_radar = radar_options[:8]
        radar_selection = st.multiselect("Choix radar (bases G/D)", radar_options, default=default_radar, label_visibility="collapsed", key=f"radar_sel_{key_suffix}")
        
        radar_labels = []
        radar_values = [] 
        radar_vals_d = []
        for base in radar_selection[:8]:
            ...
            # Remplace les lignes de calcul p_g et p_d par :
            ...
            radar_values.append(p_g)
            radar_vals_d.append(p_d)
            
        if radar_labels:
            radar_b64_preview = create_radar_chart_kine(radar_labels, radar_values, radar_vals_d)
            st.markdown(f'<div style="text-align:center;"><img src="data:image/png;base64,{radar_b64_preview}" style="width:100%; max-width:380px;"></div>', unsafe_allow_html=True)
            ...
            st.caption("Sélectionne au moins une paire de variables numériques (G/D) pour afficher le radar.")
...
with c_ent1:
    entretien_date = st.text_input("Date de l'entretien", key=f"ent_date_{key_suffix}")
with c_ent2:
    rdv_date = st.text_input("Prochain RDV / Test", key=f"rdv_date_{key_suffix}")

d1, d2 = st.columns(2)
with d1:
    dominant = st.text_area("Points forts", key=f"dominant_{key_suffix}", height=100)
    if report_mode == "Préparation Physique":
        strat_salle = st.text_area("Stratégie Salle", key=f"strat_salle_{key_suffix}", height=100)
        strat_terrain = st.text_area("Stratégie Terrain", key=f"strat_terrain_{key_suffix}", height=100)
    else:
        strat_salle = st.text_area("Stratégie de prise en charge", key=f"strat_salle_{key_suffix}", height=100)
        strat_terrain = ""
with d2:
    weak = st.text_area("Axes d'amélioration", key=f"weak_{key_suffix}", height=100)
...
if st.button("Générer le rapport HTML", type="primary"):
    ...
    if report_mode == "Préparation Physique":
        ...
        html_report = build_kine_report(
            ...
            photo_b64, logo_b64, logo_ext, staff_evals, 
            sel_session, df_prev_session, rdv_date, entretien_date, context_test,
            ref_group_label, comp_zscores, antecedents_kine, leg_overrides, radar_vals_d
        )
