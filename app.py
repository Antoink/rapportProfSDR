...
from planning import JOURS, MOMENTS, CATEGORIES, empty_planning, build_planning_html, build_kpi_department_plan_html
...
kpi_info = get_kpi_auto_fill(p_sel)
...
with c_auto1:
    if kpi_info:
        ...
        st.caption(
            ...
            "(nom absent ou ambigu dans le document source — complète `PLAYER_KPI_PRIORITAIRE` "
            "dans `config.py` si besoin)."
        )
with c_auto2:
    if st.button("🔄 Pré-remplir", disabled=not kpi_info, use_container_width=True,
                 help="Remplit Points forts / Axes d'amélioration / Stratégies à partir du plan "
                      "d'individualisation et de la meilleure performance mesurée. Reste éditable "
