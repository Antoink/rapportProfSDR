...
def _build_asymmetry_block(selected_metrics, df_full, row, use_relative, lateralite, leg_overrides):
    ...
    <div style="margin-bottom:12px;">
        ...
        <div style="display:flex; flex-wrap:wrap; gap:6px;">{''.join(blocks)}</div>
    </div>
    """
...
def _build_isak_block(row, df_full, selected_metrics):
    ...
    return f"""
    <div class="no-break" style="margin-bottom:16px;">
        <div style="font-weight:900; color:{SDR_RED}; font-size:10pt; text-transform:uppercase; border-bottom:2px solid {SDR_RED}; padding-bottom:4px; margin-bottom:12px;">COMPOSITION CORPORELLE — PROTOCOLE ISAK (PLIS CUTANÉS, MM)</div>
        <div style="display:flex; justify-content:space-between; align-items:center;">
            <div style="width:50%; text-align:center;">
...
def build_kine_report(
    ...
    radar_html = (
        f'''<div class="no-break" style="text-align:center; margin: 20px 0;">
